...
class MailChimp(object):
    ...
    def subscribe(contact_email, contact_first_name, contact_last_name):
        subscription_body = {
            ...
            "status": "pending",
            "merge_fields": {"FNAME": contact_first_name, "LNAME": contact_last_name,},
        }
        ...
        if subscription_response.status_code == 200:
            ...
            check = requests.get(
                "https://us3.api.mailchimp.com/3.0/lists/{}/members/{}".format(settings.MAILCHIMP_LIST_ID, member_id)
            )

            if check.status_code != 200:
                logging.error(
                    "Subscription call was successful, but member '{}' does not appear to exist".format(contact_email)
                )
        else:
